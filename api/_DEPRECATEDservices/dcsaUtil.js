...
const itemName = 'dcsa';
const modelName = 'dcsa';
const itemIdName = 'codigodcsa';
const xmlFile = 'data/DICCIONARIO_DCSA.xml';
const endCollection = 'aemps_prescripcion_dcsa';
...
module.exports.update = function () {
  return new Promise(function (resolve, reject) {
    ...
    sails.log.info("[CRON] - [DCSA] - Updating DCSA...");


    //Get all Collection IDs
    Dcsa.native(function (err, collection) {
      if (err) reject(err);
      collection.aggregate([{$group: {_id: null, ids: {$addToSet: "$_id"}}}]).toArray(function (err, results) {
        ...
        xml.on('endElement: ' + itemName, function (item) {
          ...
          Dcsa.findOne({_id: item._id}).exec(function (err, oldItem) {
            if (err) reject(err);

            Dcsa.native(function (err, collection) {
              if (err) reject(err);

              collection.updateOne({codigodcsa: id}, {$set: item}, {upsert: true}, function (err, results) {
                if (err) reject(err);
        ...
        xml.on('endElement: ' + endCollection, function () {
          ...
          Dcsa.native(function (err, collection) {
            ...
            deletedIds.forEach(function (entry) {
              Dcsa.findOne({_id: entry}).exec(function (err, beforeDelete) {
                if (err) reject(err);
