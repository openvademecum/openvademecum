...
const itemName = 'envases';
const modelName = 'envases';
const itemIdName = 'codigoenvase';
const xmlFile = 'data/DICCIONARIO_ENVASES.xml';
const endCollection = 'aemps_prescripcion_envases';
...
module.exports.update = function () {
  return new Promise(function (resolve, reject) {
    ...
    sails.log.info("[CRON] - [Envases] - Updating Envases...");


    //Get all Collection IDs
    Envases.native(function (err, collection) {
      if (err) reject(err);
      collection.aggregate([{$group: {_id: null, ids: {$addToSet: "$_id"}}}]).toArray(function (err, results) {
        ...
        xml.on('endElement: ' + itemName, function (item) {
          ...
          Envases.findOne({_id: item._id}).exec(function (err, oldItem) {
            if (err) reject(err);

            Envases.native(function (err, collection) {
              if (err) reject(err);

              collection.updateOne({codigoenvase: id}, {$set: item}, {upsert: true}, function (err, results) {
                if (err) reject(err);
        ...
        xml.on('endElement: ' + endCollection, function () {
          ...
          Envases.native(function (err, collection) {
            ...
            deletedIds.forEach(function (entry) {
              Envases.findOne({_id: entry}).exec(function (err, beforeDelete) {
                if (err) reject(err);
