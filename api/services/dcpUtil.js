...
const itemName = 'dcp';
const modelName = 'dcp';
const itemIdName = 'codigodcp';
const xmlFile = 'data/DICCIONARIO_DCP.xml';
const endCollection = 'aemps_prescripcion_dcp';
...
module.exports.update = function () {
  return new Promise(function (resolve, reject) {
    ...
    sails.log.info("[CRON] - [DCP] - Updating DCP...");


    //Get all Collection IDs
    Dcp.native(function (err, collection) {
      if (err) reject(err);
      collection.aggregate([{$group: {_id: null, ids: {$addToSet: "$_id"}}}]).toArray(function (err, results) {
        ...
        xml.on('endElement: ' + itemName, function (item) {
          ...
          Dcp.findOne({_id: item._id}).exec(function (err, oldItem) {
            if (err) reject(err);

            Dcp.native(function (err, collection) {
              if (err) reject(err);

              collection.updateOne({codigodcp: id}, {$set: item}, {upsert: true}, function (err, results) {
                if (err) reject(err);
        ...
        xml.on('endElement: ' + endCollection, function () {
          ...
          Dcp.native(function (err, collection) {
            ...
            deletedIds.forEach(function (entry) {
              Dcp.findOne({_id: entry}).exec(function (err, beforeDelete) {
                if (err) reject(err);
