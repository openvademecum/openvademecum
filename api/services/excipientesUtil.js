...
const itemName = 'excipientes';
const modelName = 'excipientes';
const itemIdName = 'codigoedo';
const xmlFile = 'data/DICCIONARIO_EXCIPIENTES_DECL_OBLIGATORIA.xml';
const endCollection = 'aemps_prescripcion_excipientes';
...
module.exports.update = function () {
  return new Promise(function (resolve, reject) {
    ...
    sails.log.info("[CRON] - [Excipientes] - Updating Excipientes...");


    //Get all Collection IDs
    Excipientes.native(function (err, collection) {
      if (err) reject(err);
      collection.aggregate([{$group: {_id: null, ids: {$addToSet: "$_id"}}}]).toArray(function (err, results) {
        ...
        xml.on('endElement: ' + itemName, function (item) {
          ...
          Excipientes.findOne({_id: item._id}).exec(function (err, oldItem) {
            if (err) reject(err);

            Excipientes.native(function (err, collection) {
              if (err) reject(err);

              collection.updateOne({codigoedo: id}, {$set: item}, {upsert: true}, function (err, results) {
                if (err) reject(err);
        ...
        xml.on('endElement: ' + endCollection, function () {
          ...
          Excipientes.native(function (err, collection) {
            ...
            deletedIds.forEach(function (entry) {
              Excipientes.findOne({_id: entry}).exec(function (err, beforeDelete) {
                if (err) reject(err);
